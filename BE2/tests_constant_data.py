...
for i in omegas_tests:
    ...
    path = "spoken_digit_dataset2/"
    duree_trame = 0.030
    #ordre = 10
    omega_v, omega_d, omega_h = i[0], i[1], i[2]
    ...
    def get_r_vector(window):
        ...
        for index in range(ordre + 1):
            ...
            vect_2 = window[index:]
            r_vector.append(np.sum(vect_1 * vect_2) / len(vect_1))
        return r_vector
    ...
    def get_a_vector(matrix):
        ...
        sigma_vector[0] = 1
        a_vector = np.linalg.solve(matrix, sigma_vector)
        a_vector = a_vector / a_vector[0]
        return a_vector[1:]
    ...
    sample_size = 400 # maximum 2000 (déconseillé sur pc ordinaire)
    ...
    def chargement_donnees():
        ...
        for chiffre in range(10):
            ...
            limit = int(0.8*len(names))
            donnees = donnees + names[:limit]
            tests = tests + names[limit:]

        donnees = donnees + tests
    ...
    plt.xlabel('K')
    plt.ylabel('Accuracy')
    plt.title('Accuracy(K) avec (ordre = ' + str(ordre) + ', oh = ' + str(omega_h) + ', od = ' + str(omega_d) + ', ov = ' + str(omega_v) + ')')
    plt.plot(les_k, les_accuracy, 'ro')
    plt.savefig("accuracies_ordre_" + str(ordre) + "_omegaV_" + str(omega_v) + "_omegaH_" + str(omega_h) + "_omegaD_" + str(omega_d) + ".png")
    plt.close('all')
